...
\par Having low SNR already makes Eve's job of making sense of the communication harder as she starts to gets decoding errors. However in the usual 16-QAM constellation (4-PAM symboling set) for example, the codewords are arranged in a spesific manner (see figure \ref{fig:16QAM}) and the decoding errors Eve gets are usually decoding to the neighboring codeword. For example if Alice sent 1111 to Bob using the codebook in figure \ref{fig:16QAM}, Eve probably won't decode to 0000 even with low SNR. So in the end Eve gets a rough idea what was sent by analyzing the codeword distribution. 
\par The goal is to make such statistical analysis close to impossible, i.e. all Eve can do is take a guess. One way to achieve this is so called coset coding. Instead of assinging all 16 codewords in figure \ref{fig:16QAM} a different message one can assign the same message to multiple codewords. For example one can decide that the codewords in set $\{0001, 0010, 1101, 1110\}$ all mean the same message in figure \ref{fig:16QAM}. This set of codewords belongs to a sublattice, in this example $2\mathbb{Z}^2$, of the original lattice ($\mathbb{Z}^2$) in which the constellation lies in. 
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{16-QAM}
  \caption{The 16-QAM codeword constellation with Gray encoding, each point represents a codeword. Using this scheme the codewords can store up to 4 bits of information if each codeword is assigned a unique message.}
  \label{fig:16QAM}
\end{figure}
...
\begin{figure}[ht]
  ...
  \caption{Five different sublattices for coset encoding in wiretap simulations. ECDP stands for "Eve's correct decoding probability".}
  \label{fig:ecdp}
\end{figure}
