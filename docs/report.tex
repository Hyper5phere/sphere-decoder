...
\par The codebook is then used for calculating its average and maximum energy (Frobenius norm of the codeword matrix $\mathbf{X}$). The codebook average energy is later used in calculating the variance for the channel matrix. After all preparations are done, the program prints out the info relevant to current simulation and asks the user to verify it before starting the actual simulations.
