...
\par One can optionally make use of the computing power of the GPU which could potentially speed up large matrix operations that the linear algebra library Armadillo \cite{arma} does. This requires Cuda toolkit and NVBLAS, see details here: \url{http://docs.nvidia.com/cuda/nvblas/index.html}. When using this you also need to add an extra compiler flag in the Makefile before building the program.
\par Before trying to install and run the program itself, you need to make sure you have all the required libraries and a compatible C++ compiler installed. The recommended GNU C++ compiler (g++) should be preinstalled in most Linux distributions but in case it is not, please refer to installation notes here: \url{https://gcc.gnu.org/wiki/InstallingGCC}. 
