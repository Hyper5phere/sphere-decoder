...
\begin{tikzpicture}[node distance = 5em, auto]
    ...
    \node [block, right of=estimate, node distance=9.5em,align=center] (radius) {Find $r^2$ so that\\ size of spherical\\ codebook is within\\ range $\left[2^s, 2^{s+1}\right]$};
    ...
    \node [decision, right of=siminfo, node distance=9em] (continue) {Continue to simulation?};
    \node [cloud, right of=continue, align=center, node distance=9em] (exit) {Exit\\ program};
    \node [block, below of=continue, node distance=7em, align=center] (startsim) {Start parallel\\ SNR simulations\\ $\rightarrow$ for each SNR do};
    ...
    \node [block, right of=startsim, node distance=10em, align=center] (channel) {Calculate channel\\ matrix $\mathbf{H}$ from $\tau^2$\\ according to channel\\ model};
    ...
    \node [block, below right of=loop, node distance=9em, align=center] (end) {Save simulation\\ results and wait for\\ other simulations to finish};
    \node [cloud, below right of=end, node distance=7.5em, align=center] (finish) {Output all simulation\\ results (csv) and exit program};
    ...
    \path [line] (spherical) -- node {no} (codebook);
    \path [line] (estimate) -- node {yes} (radius);
    \path [line] (estimate) -- node {no} (codebook);
    ...
    \path [line] (continue) -- node {yes} (startsim);
...
\end{tikzpicture}
\caption{The program flow chart.}
\label{fig:flowchart}
\end{figure}
