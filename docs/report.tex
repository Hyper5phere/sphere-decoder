...
As an example, let's consider figure \ref{fig:16QAM} where the codebook is carved out of lattice $\mathbb{Z}^2$, so $\Lambda_b = \mathbb{Z}^2$. An intuitive way to pick the sublattice is $\Lambda_e = 2\mathbb{Z}^2$, so now for example the codewords $\{0001, 0010, 1101, 1110\}$ all mean the same message. The right neighbors of those codewords (affine shift) form another coset that represents a different message and so on. 
...
\end{equation}
so in the given example this rate is only 2-bits. But the code rate $r=\log_2{|\mathcal{L}|}$ is still 4-bits, so the extra 2-bits are being used for confusion. That is why the difference $r-r_i$ is called the confusion rate $r_c$.
\begin{figure}[ht]
  \centering
