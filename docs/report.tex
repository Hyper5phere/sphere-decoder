...
Now that the program is installed it is time to take a look at the simulation parameters and the program input in order to run successful simulations. In the /bases/ folder there are text files containing the lattice code basis matrices for the simulation. These files are really important as they contain the lattice code itself, the performance of which you want to simulate. You can add your own basis matrix text files there by following a spesific format, which is later discussed in detail with examples. You can use the example basis matrix files provided with the program as a reference.
...
{0+I, 0}}
\end{Verbatim}
\caption{An example basis matrix input file (alamouti.txt) for the Alamouti lattice code.}
