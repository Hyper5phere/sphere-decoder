...
\label{eq:cvp}
\end{equation}
This can be thought as traversing the search tree of depth $m$ from root node to the leaf node at lowest level with smallest path weights possible i.e. finding the shortest path. In the figure \ref{fig:tree} a search tree for 4-dimensional CVP is illustrated. The tree is not full as not all subtrees need to be considered in smart implementations of the lattice search algorithm as the search can be stopped early which we're soon to find out. Please note that only paths from root to level 4 nodes are considered feasible solutions to CVP.
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.5\linewidth]{search-tree}
  \caption{Travelsal of a lattice search algorithm in four dimensional lattice.}
  \label{fig:tree}
\end{figure}
