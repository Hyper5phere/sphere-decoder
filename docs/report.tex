...
\begin{tikzpicture}[node distance = 5em, auto]
    % Place nodes
    \node [block, align=center] (main) {program start\\ and setup};
    \node [block, left of=main, align=center, node distance=9em] (choose) {Choose channel\\ model and data\\ encoding method};
    \node [decision, below of=main] (estimate) {Estimate squared radius?};
    \node [decision, left of=estimate, node distance=9em] (spherical) {Use spherical shaping?};
    \node [block, right of=estimate, node distance=9.5em,align=center] (radius) {Find $r^2$ s.t.\\ size of spherical\\ codebook is within\\ range $\left[2^s, 2^{s+1}\right]$};
    ...
    %\node [block, below of=main] (init) {configure()};
    %\node [cloud, left of=init] (expert) {input};
